...
func (r *projectRepoPg) GetMembers(projectID string) ([]*userdomain.User, error) {
    query := `
        SELECT u.id, u.name, u.email, u.role, u.timezone, u.language, u.created_at, u.updated_at
        FROM users u
    ...
    for rows.Next() {
        ...
        err := rows.Scan(&user.ID, &user.Name, &user.Email, &user.Role, &user.Timezone, &user.Language, &user.CreatedAt, &user.UpdatedAt)
        if err != nil {
            return nil, err
        }
